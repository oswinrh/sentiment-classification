...
function updatewordnet($queryuji)
{
	...
	while($insert1 = mysql_fetch_array($que_insert1))
	{
		...
		$queryidplus2 = mysql_query("SELECT id_token FROM tokenwordnet ORDER BY id_token DESC LIMIT 1");
		$getidplus2 = mysql_fetch_array($queryidplus2);
		$next_id = $getidplus2['id_token'] + 1;

		mysql_query("INSERT INTO tokenwordnet(id_token,id_dok,id_kelas,token,tf,df,idf,w,normalw,tag) VALUES('$next_id','$idnyadok','$kelasnya','$tokenya','$tfnya','','','','','latihbaru')");
	...
	$qtokenlatih = mysql_query("SELECT id_token,id_dok,id_kelas,token,tag,tf FROM tokenwordnet");
	...
	{
		$qdflatih = mysql_query("SELECT id_dok FROM tokenwordnet WHERE token = '$value'");
	...
	{
		$queryn = mysql_query("SELECT id_dok FROM tokenwordnet GROUP BY id_dok");
	...
	{
		$qtflatih = mysql_query("SELECT tf FROM tokenwordnet WHERE id_token = '$key'");
	...
	foreach ($tempwlatih as $key => $value)
	{
		...
		//mysql_query("UPDATE token SET df ='$tempdflatih[$key]' ,idf = '$tempidflatih[$key]', w = '$value', normalw = '$normlatih' WHERE id_token = '$key'");
		//echo "UPDATE bobot SET df ='$tempdf[$key]' ,idf = '$tempidf[$key]', w = '$value', normalw = '$norm' WHERE id_token = '$key'"."<br>";

	}

	unset($arrayupdate[0]);
	$tokenresult = fopen("D:/wordnet.txt", "w");
	foreach ($arrayupdate as $key => $value)
	{
		$tokenresult = fopen("D:/wordnet.txt", "a");
		$tokentxt = $value.PHP_EOL;
		fwrite($tokenresult, $tokentxt);
		fclose($tokenresult);
	}
	mysql_query("DELETE FROM tokenwordnet");
	mysql_query("LOAD DATA INFILE 'D:/wordnet.txt' INTO TABLE tokenwordnet FIELDS TERMINATED BY ','");
}
